    ...
    return (
        <div>
            <div className={tw`flex flex-col items-center`}>
                ...
                <strong className={tw`mt-4 text-xs md:text-sm lg:text-3xl tracking-wide text-gray-900`}>Welcome to the {labContext.name}</strong>
                <p className={tw`indent-8 text-gray-600 text-xs md:text-sm lg:text-2xl text-left whitespace-normal tracking-normal leading-relaxed w-3/4`}>
            ...
            <div className={tw`lg:flex`}>
                    ...
                    <Swiper
                        ...
                        >
                        {
                            ...
                            return (
                                ...
                                <div className={tw`grid grid-col-1 md:ml-40 md:w-7/12`}>
                                    <img src={research.image}/>
                                    <ReactMarkdown className={tw`indent-8 text-xs md:text-sm lg:text-[18px]`}>Hear about our research: Dr. Li will deliver an invited talk at iNOW2023 on ”Probing the Brain Using Low-coherence Light” in Wuerzburg, Germany.</ReactMarkdown>
                                </div>
                                </SwiperSlide>
                            );
                            })
                ...
                <div className={tw`container md:px-6 mt-2`}>
                    <section className={tw`text-left`}>
                        <h2 className={tw`md:mb-2 text-xs md:text-sm lg:text-xl font-bold`}>Latest News</h2>
                        {
                            newsList.slice(0,4).map((news, index) => {
                                return (
                                    <div className={tw`md:mb-2 flex flex-wrap md:w-3/5`} key={index}>
                                        <div className={tw`mb-2 mr-auto basis-auto px-3 md:mb-0`}>
                                            <small className={tw`font-bold`}>{news.date.toISOString().slice(0, 10)}    </small>
                                            <span className={tw`text-sx md:text-neutral-500`}>
                                                {news.content}
                                            </span>
                                        </div>
                                    </div>
                        ...
                        <NavLink to="/news" className={tw`text-xm md:text-sm font-medium italic text-center text-blue-600 underline`}>
                            Read More
                        </NavLink>        
                    </section>
                </div>
